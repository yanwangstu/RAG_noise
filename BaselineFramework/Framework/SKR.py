...
class SKR:
    ...
    def inference(self, question: str, retrival_doc: list[str]) -> dict:
        first_messages = self.generator.message_generate_additionINFO(question)

        if not self.useAPI:
            # first request: 
            # Ask: Do you need additional information to answer this question?
            output_add = self.generator.generator(first_messages, False)
            output_add: GeneratorOutput

            # check if need additional information (retrieve docuents)
            if "Yes, I need." in output_add.output_text:
                need_docs = True
            ...
            # second request: 
            output = self.generator.generator(second_messages, False)
            output: GeneratorOutput
            
            return {"Output Answer": output.output_text, 
                    "Middle Answer": output_add.output_text,
                    "Need Docs": need_docs}
        
        else:
            # first request: 
            # Ask: Do you need additional information to answer this question?
            output_add = GeneratorAPI(first_messages, self.model_name)
            output_add: str

            # if need additional information (retrieve docuents)
            if "Yes, I need." in output_add:
                need_docs = True
                second_messages = self.generator.message_generate_baseRAG(question, retrival_doc)
            else:
                second_messages = self.generator.message_generate_base(question)
            ...
            # second request: 
            output = GeneratorAPI(second_messages, self.model_name)
            output: str

            return {"Output Answer": output,
                    "Middle Answer": output_add,
                    "Need Docs": need_docs}
